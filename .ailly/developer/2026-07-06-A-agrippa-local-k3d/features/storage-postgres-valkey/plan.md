...
### Step 3 build-time findings (2026-07-08)

Two live-verified fixes beyond the paper plan, both scoped to this
feature-step's own files (not cross-feature-step, so applied directly
rather than escalated):

1. **`apps/storage.yaml` needed `compare-options: ServerSideDiff=true`.**
   Once the `Cluster` synced, `storage` went `OutOfSync` (only resource:
   `Cluster postgres`) even though every field in git matched the live
   object -- CNPG's mutating webhook defaults many `Cluster.spec` fields
   (`bootstrap.initdb`, `affinity`, `monitoring`, `postgresql.parameters`,
   etc.) that are never set in git. Root cause: `storage`'s own
   `syncOptions: ServerSideApply=true` (from Step 0) auto-enables ArgoCD's
   Structured Merge Diff strategy application-wide, which predicts live
   state via a local simulation rather than a real dry-run and mispredicts
   controller-defaulted fields -- the exact same mechanism
   `apps/core.yaml` already documents and fixes for its own
   Gateway/HTTPRoute `OutOfSync` symptom (`argoproj/argo-cd#22151`), which
   Step 3's own edge case anticipated ("resolve with a narrowly-scoped
   `ignoreDifferences` and/or `compare-options: ServerSideDiff=true`
   ... mirroring `apps/core.yaml`'s precedent"). Fixed by adding the
   identical annotation to `apps/storage.yaml`; live-verified `storage`
   went `Synced` immediately after.
2. **The Valkey `helmCharts:` sub-kustomization needed a `patches:`
   workaround for a real kustomize regression.** After the
   `ServerSideDiff` fix, `storage` was `Synced` but stuck `Progressing`
   with zero Valkey resources in the `storage` namespace -- all five
   (`ServiceAccount`, `ConfigMap`, `Service`, `PersistentVolumeClaim`,
   `Deployment`) had landed in `argocd`, the Application's
   `destination.namespace` fallback, instead. Root cause, confirmed via
   local `kustomize build` reproduction and `research:public`
   (`kubernetes-sigs/kustomize#6058`): kustomize 5.8.0+ (pinned version
   here: 5.8.1) stopped stamping `metadata.namespace` onto resources
   generated by `helmCharts:` inflation when the chart's own templates
   don't hardcode `{{ .Release.Namespace }}` (`valkey-io/valkey-helm`'s
   don't) -- the top-level `namespace:` transformer field silently no-ops
   for such resources, and `helmCharts[].namespace: storage` only sets
   `.Release.Namespace` for in-template use, not the emitted objects'
   actual `metadata.namespace`. Fixed with the maintainer-acknowledged
   workaround from the issue thread: a `patches:` block in
   `storage/overlays/dev/valkey/kustomization.yaml` targeting exactly the
   five kinds this chart emits with a JSON6902 `add /metadata/namespace`
   op -- scoped to this sub-kustomization alone, so it cannot touch
   `cnpg-operator/`'s separately-accumulated resources. Live-verified: a
   hard refresh landed all five Valkey resources correctly in `storage`,
   the stale `argocd`-namespace copies were pruned automatically, and
   `storage` reached `Synced Healthy` (`Cluster.status.phase`: "Cluster in
   healthy state"; Valkey pod `Running`; PVC on `local-path`).
