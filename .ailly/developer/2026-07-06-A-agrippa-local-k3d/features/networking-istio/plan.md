...
- Edge case (the design's flagged near-certain hit): if the CNI override values are wrong or absent, `ztunnel`/workload pods lose networking or `istio-cni` node-agent CrashLoops — verify live with `kubectl -n istio-system logs -l k8s-app=istio-cni-node` and `docker exec k3d-agrippa-dev-server-0 ls /var/lib/rancher/k3s/data/cni` before trusting green.
