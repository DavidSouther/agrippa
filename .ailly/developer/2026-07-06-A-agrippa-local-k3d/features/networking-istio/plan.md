...
- Edge case found during build, not anticipated in the design/plan: `Gateway` and `HTTPRoute` both carry a registered `status` subresource that istiod's Gateway API controller populates (`Programmed`/`Accepted` conditions, addresses, `attachedRoutes`) — none of which the git-sourced, spec-only manifest ever sets, leaving `core` permanently `OutOfSync` once the Gateway landed even though `spec` matched exactly (including API-defaulted fields like `certificateRefs[].group`/`kind`). Fixed the same way as Step 2's webhook-caBundle finding: two more `spec.ignoreDifferences` entries on `apps/core.yaml`, this time ignoring the whole `/status` subresource for the named `Gateway agrippa-gateway` and `HTTPRoute argocd` — standard practice for controller-managed status, and it does not affect the feature test's own live `kubectl get ... -o jsonpath='{.status...}'` reads.
