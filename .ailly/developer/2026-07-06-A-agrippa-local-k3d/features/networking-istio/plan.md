...
- [x] Step 5: Feature-test issuer-check correction (Q6), full GREEN, and the regression sweep
...
- Edge case found during build, not anticipated in the design/plan: `Gateway` and `HTTPRoute` stayed permanently `OutOfSync` once the Gateway landed, even though `spec` matched the live objects byte-for-byte. Three `ignoreDifferences` mechanisms were tried in turn and, live-verified this session, none cleared it: `jsonPointers: [/status]`, `jqPathExpressions: [.status]`, then `managedFieldsManagers` (confirmed via `--show-managed-fields` as the technically-correct owner set). The actual root cause, found via `research:public` against ArgoCD's v3.4.4 source and a matching still-open upstream issue (argoproj/argo-cd#22151): `syncOptions: ServerSideApply=true` (needed application-wide for the large CRD manifests, Q4) silently auto-enables ArgoCD's deprecated "Structured Merge Diff" comparison strategy, which predicts the live state via a local simulation rather than a real API-server dry-run, and mispredicts CRD-defaulted/controller-owned fields -- so all three `ignoreDifferences` attempts were solving the wrong layer; ArgoCD's diff ALGORITHM was wrong, not merely un-ignored content. Fix: the Application annotation `argocd.argoproj.io/compare-options: ServerSideDiff=true`, which forces the stable, real-dry-run diff strategy instead; the three `ignoreDifferences` entries for Gateway/HTTPRoute were removed as unnecessary (ArgoCD's own default `resourceOverrides["*/*"]` already ignores `/status` for every resource, which is also why the earlier `jsonPointers`/`jqPathExpressions` attempts looked redundant rather than obviously wrong). A second, unrelated genuine diff was found alongside it: `HTTPRoute.spec.rules[0].matches` is API-schema-defaulted to `[{path: {type: PathPrefix, value: "/"}}]` when omitted, and ArgoCD's SMD-era pre-sync diff did not replicate that nested-array default the way it replicated scalar defaults (e.g. `backendRefs[].group`/`kind`) — fixed by authoring `matches` explicitly in `argocd-httproute.yaml` rather than relying on the diff engine to predict it (this fix is independent of the compare-options fix and stays regardless). Neither fix affects the feature test's own live `kubectl get ... -o jsonpath='{.status...}'` reads.
