...
| [secret-rotation.md](./secret-rotation.md) | You need to rotate the `age` keypair, or (more commonly) you're tempted to run `rotate-keys` to fix a placeholder value in `.sops.yaml` -- don't; read this first. Documents a real, unfixed bug in the rotation script's own test suite. |
